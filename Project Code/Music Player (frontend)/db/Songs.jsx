...
function Songs() {
  ...
  // ✅ Fetch Data Only Once (Prevents Infinite API Calls)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    items.forEach((item) => {
      ...
      if (audioElement) {
        audioElement.addEventListener('play', () => handleAudioPlay(audioElement));
      }
    });
    ...
    return () => {
      items.forEach((item) => {
        const audioElement = document.getElementById(`audio-${item.id}`);
        if (audioElement) {
          audioElement.removeEventListener('play', () => handleAudioPlay(audioElement));
        }
      });
    };
  }, [currentlyPlaying]);
                ...
                <div className="card h-100">
                  ...
                  <div className="card-body">
                    <div className="d-flex justify-content-between align-items-center mb-2">
                      ...
                      {isItemInWishlist(item.id) ? (
                        <Button variant="light" onClick={() => removeFromWishlist(item.id)}>
                          <FaHeart color="red" />
                        </Button>
                      ) : (
                        <Button variant="light" onClick={() => addToWishlist(item.id)}>
                  ...
                  <div className="card-footer d-flex justify-content-center">
                    {isItemInPlaylist(item.id) ? (
                      ...
                      <Button variant="outline-primary" onClick={() => addToPlaylist(item.id)}>
                        Add to Playlist
                      </Button>
                    )}
                  </div>
